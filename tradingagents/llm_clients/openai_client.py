...
def _resolve_provider_base_url(provider: str) -> Optional[str]:
    ...
    Ollama supports ``OLLAMA_BASE_URL`` and Xiaomi supports
    ``XIAOMI_BASE_URL``. The check is call-time, not import-time, so tests
    ...
    if provider == "ollama":
        ...
        if env_url:
            return env_url
    if provider == "xiaomi":
        env_url = os.environ.get("XIAOMI_BASE_URL")
        if env_url:
            return env_url
